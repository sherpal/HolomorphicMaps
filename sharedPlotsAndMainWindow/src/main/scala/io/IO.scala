...
import nodejs.{Buffer, FileSystem, Path}
...
object IO {
  ...
  def errToThrowable(err: Any): Throwable = err match {
  ...
  def writeFile(directory: String, buffer: Buffer, errorCallback: (Throwable) => Any = (_) => {}): Unit = {
    try {
      FileSystem.writeFile(Path.join(baseDirectory, directory), buffer, (error: js.Error) => {
        errorCallback(errToThrowable(error))
      })
    } catch {
      case err: Throwable => errorCallback(err)
    }
